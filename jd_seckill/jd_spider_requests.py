    ...
    def get_headers(self):
        return {"User-Agent": self.user_agent,
                "Accept": "text/html,application/xhtml+xml,application/xml;"
                          ...
                          "v=b3",
                #默认是keep-alive，可能没有释放，加参数Connection:close
...
class JdTdudfp:
    ...
    async def _get_auto_eid_fp(self):
        ...
        try:
            ...
            # pyppeteer 是一个Node.js库，用于远程控制无头chrome 或Chromium浏览器。 它可以自动执行各种web任务，例如导航，设置cookie、提取数据
            from pyppeteer import launch
            url = "https://www.jd.com/"

            # launch 用于配置一些浏览器启动选项，禁用沙箱和设置用户代理以获得更好的兼容性
            ...
            await page.goto(a_href)
            # 导航点击 添加到购物车按钮 导航到添加到购物车按钮并单机它以将商品添加到购物车
            ...
            for _ in range(30):
                ...
                # 提取fp 尝试使用 page.evaluate 从js中提取FP。 如果成功，它将记录此值并将结构返回
            ...
            # 如果在执行过程中发生错误，它将打印错误消息并配置文件中返回值。这样做是为了避免不同账户具有相同FP的问题
